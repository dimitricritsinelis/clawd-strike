"""Export or render the isolated near-face preview without installing an asset."""
import argparse
import hashlib
import json
import math
...
OUT = Path(__file__).resolve().parent / 'exports/near-face-grip'
parser = argparse.ArgumentParser()
parser.add_argument('mode', choices=['export','frames'])
args = parser.parse_args(sys.argv[sys.argv.index('--')+1:])
scene = bpy.context.scene
if Path(bpy.data.filepath).resolve() != OUT/'near-face-reload-preview.blend':
...
if args.mode == 'export':
    ...
    for ob in [root,*root.children_recursive]:
        ob.select_set(True)
    path = OUT/'near-face-reload-preview.glb'
    ...
    for mesh in scene.objects:
        keys = mesh.data.shape_keys if mesh.type == 'MESH' else None
