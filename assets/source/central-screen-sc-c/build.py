"""Build the finite SC-C closed screen, in metres. Front -Y, rear plane Y=0.

blender --background --factory-startup --python build.py -- --root REPO [--clay]
The editable source retains named joinery. Only the SC-C assembly is exported.
"""
import argparse
import hashlib
import json
...
materials = []
dependencies = []
for name, multiplier in [('SC-C | oiled timber and iron', 1), ('SC-C | closed dark backing', .24)]:
...
for side, low, high in [('left', -.408, -.010), ('right', .010, .408)]:
    ...
    for direction in [-1, 1]:
        ...
        count = math.ceil((intercept_max - intercept_min) / .080)
        for i in range(count):
            intercept = intercept_min + .04 + i * .080
            points = []
            for x in [xmin, xmax]:
                z = slope * x + intercept
                if zmin <= z <= zmax:
            ...
            if len(points) != 2 or (points[1] - points[0]).length < .025:
                continue
            a, b = points
            bar = box(f'{side} lattice {direction:+d} {i:02d}', (a + b) / 2, (.014, .016, (b - a).length), bevel=0)
...
for x in [-.455, .455]:
    for z in [.14, 1.30]:
        ob = bpy.data.objects.new(f'SC-C | mount corner {x:+.3f} {z:.2f}', None)
...
for image in bpy.data.images:
    if image.source == 'FILE':
        image.pack()
bpy.ops.wm.save_as_mainfile(filepath=str(SOURCE / 'central-screen-sc-c.blend'))

clay = bpy.data.materials.new('Review clay')
clay.use_nodes = True
clay.node_tree.nodes.get('Principled BSDF').inputs['Base Color'].default_value = (.50, .48, .44, 1)
...
for name, location, power, size in [('soft daylight', (-3, -4, 5), 500, 4), ('fill', (3, -1, 2), 150, 3), ('rear', (0, 3, 3), 300, 3)]:
...
scene.camera.data.ortho_scale = 1.8
for name, position in [('front', (0, -4, 1.0)), ('approach', (2.6, -4, 2)), ('rear', (-2.6, 4, 1.8))]:
    scene.camera.location = position
    scene.camera.rotation_euler = (Vector((0, -.07, .70)) - scene.camera.location).to_track_quat('-Z', 'Y').to_euler()
    scene.render.filepath = str(SOURCE / f'{"clay" if args.clay else "material"}-{name}.png')
    bpy.ops.render.render(write_still=True)
if args.clay:
    print('CLAY REVIEW COMPLETE; no GLB exported')
    sys.exit(0)
...
glb = OUT / 'central-screen-sc-c.glb'
