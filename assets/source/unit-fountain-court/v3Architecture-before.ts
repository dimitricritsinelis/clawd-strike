...
function pushMassing(
  ...
  const identity = resolveBuildingMaterialIdentity(placement, profile.family);
  // An authored facade GLB owns the face: keep the shell, roof edge and roof,
  // skip the kit's base band, apron, repair patches and corner piers.
  const faceOwnedByModel = Boolean(placement.facadeModelId);
...
export function buildV3Architecture(options: BuildV3ArchitectureOptions): V3ArchitectureBuildResult {
  ...
  const facadeOwnedFrontageIds = new Set(options.placements.flatMap((placement) =>
    placement.kind === "massing" && placement.facadeModelId ? [placement.frontageId] : []));
