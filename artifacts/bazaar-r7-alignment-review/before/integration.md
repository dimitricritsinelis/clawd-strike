...
**R6 design review:** read the current per-face window decisions for the requested area and confirm the source hash before construction. A REVISE face is not ready. This is the user-requested design check; after the approved handoff is frozen, implement continuously and perform the usual consolidated end review. Do not invoke another wall-review subagent during routine mesh construction.
...
Build one requested area or an explicit user-provided queue. BZ-04 authorizes design across the map, not an automatic all-map implementation or rollout. The first R6 implementation sample should validate the common craft/material approach and its declared neighboring context before it is repeated elsewhere. Installed B/shared assets may be reused where the frozen handoff confirms compatibility; the first-use shared-environment prerequisite must be explicit in any task that lacks that installation.
