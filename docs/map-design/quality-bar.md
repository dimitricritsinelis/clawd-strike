...
Implementation follows the [canonical full-map queue](construction/README.md#approved-whole-map-queue-and-readiness): read the area’s engineering documents and dependencies once, build and integrate the complete area, perform one consolidated review, make targeted corrections and continue automatically. Final user art acceptance is separate and does not require routine approval between areas. Do not insert mid-build aesthetic reviews or recurring signoff cycles. Preserve correctness and gameplay checks; run them at the end and rerun only affected checks after a correction.

For the current R7 task, defer performance benchmarking and optimization to a separate task after all 25 areas and shared scope are implemented. Triangle, material, primitive, shadow and draw-call budgets remain numerical reference targets and are informational during construction. Record readily available counts without budget approval loops or new optimization work. Use the existing `bz04-trial.mjs --capture-only` path for in-game review; retain geometry validity, bounds, clearance, load-error and gameplay-identity checks. Deferred performance does not block construction completion and must not be reported as passing.
...
4. Record pass, fail, unavailable and baseline-only failures separately. Keep an area `building` or `built` until the required evidence exists; set `complete` after actual-game visual, correctness and gameplay validation. Record deferred performance and pending user art acceptance separately.
