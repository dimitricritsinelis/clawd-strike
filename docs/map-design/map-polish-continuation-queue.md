...
- [ ] Complete
- Status: Stalled 2026-08-08 after one retained clear primary-camera improvement and two consecutive cause-distinct support-depth non-improvements; both rejected refinements were fully restored.
...
- Fresh before: `artifacts/playwright/map-shots/continuation-C3-before/` — all four exact authored cameras passed with no capture findings from the accepted C1 / stalled C2 checkpoint.
- Attempt 1 after / blind result: `continuation-C3-a1-clustered-massing/`. This replaced 120 isolated regular bays with deterministic two- and three-bay party-wall runs, unequal joined footprints, four side-relative two-volume crown profiles, varied bearing heights, cluster-coherent materials, and crown-derived supported roofs/utilities while keeping all 120 shell ordinals. Repetitive per-bay chimneys and same-budget service tokens were removed rather than used to simulate structure. In the anonymous Maple/Quartz comparison, a fresh critic preferred Maple overall with medium-high confidence and called the `SHOT_01` gain clear: joined stepped footprints establish a substantially stronger low/mid/tall hierarchy and reduce the isolated-box field. `SHOT_02`, `SHOT_12`, and `SHOT_13` tied, with the playable foreground stable. This clear partial improvement was retained. A final read-only geometry audit then found crossed adjacent rings and overlapping internal courses inside that implementation; the corrected exact set is `continuation-C3-final-geometry-safe/`. It derives non-overlapping ring offsets with at least `0.18 m` lower-block separation and trims course overhangs at internal party walls. In a new anonymous Moss/Pearl comparison against the original before, the critic again chose the corrected result with high confidence (`0.86` overall, `0.94` in `SHOT_01`), found no visible interpenetration or z-fighting, and rated `SHOT_12` a slight improvement while `SHOT_02` / `SHOT_13` tied. The geometry-safe set is the retained final.
- Attempt 2 after / blind result: `continuation-C3-a2-axial-depth-layer/`. Camera-distance analysis hypothesized that two ring-one rear towers collapsed to only a few pixels behind the inner terrace, so only those towers received a bounded `2.1 m` bearing lift and wider structural crown. In the anonymous Amber/Cobalt comparison, a second fresh critic rated all four cameras ties: neither axial support gained a meaningful skyline layer. The complete refinement was restored.
- Attempt 3 after / blind result: `continuation-C3-a3-ring-two-depth/`. Exact pixel-diff localization and camera-ray mapping then identified a distinct shared ring-two slot in both axial view cones. A cause-distinct far-stratum pass raised only those two bearing blocks to `14.4 m` and guaranteed a supported crown at least 72% of bay width, without repeating the ring-one adjustment. In the anonymous Fern/Slate comparison, a third fresh critic again rated all four cameras ties: the small far silhouettes did not become perceptible depth layers. This refinement was also fully restored.
- Checks: `pnpm typecheck` and `pnpm exec tsx --test src/runtime/map/buildBlockout.background.test.ts src/runtime/map/buildBlockout.elevation.test.ts` pass after restoration/correction (7/7 focused tests). The retained planner regression proves deterministic repeatability, 120 stable sequential IDs, strict separation from the gameplay rectangle, two-/three-bay party-wall joins, real cluster alleys, non-intersecting adjacent rings, non-overlapping internal masonry courses, all four profiles and at least a `4 m` height span on every side, contained bearing crowns, and supported tank footprints. The final `SHOT_13` capture reported 631 draws, 1,621,179 triangles, and a 6.85 ms CPU-frame median, versus 631 / 1,637,595 / 6.20 ms before; the final wave `qa:completion` remains authoritative for the 12.5 ms ceiling.
- Final status / commit: Stalled with the clearly preferred Attempt 1 retained. `SHOT_01` is materially improved, but broad exposed lower-roof slabs, residual repeated rectangular caps, and sparse outer voids remain; more importantly, `SHOT_02` and `SHOT_12` still do not show the complete-when bar's multiple believable background height/depth layers. The two-rejection threshold forbids further height-stratum grinding in this wave. The checkpoint is the commit containing this record.
