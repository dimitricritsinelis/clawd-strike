...
def b_finish_notes(area):
    ...
    revision = revision if revision in ('R4','R5','R6','R7') else 'B-05'
    lines=[f"# {revision} | B courtyard finish schedule",'', ('Retained B building layout and focal balcony, with the explicitly scheduled revised windows, surrounds and finish; no gameplay changes.' if revision in ('R4','R5','R6','R7') else '**Proposed finish on the retained B-04 composition.** No new window shapes, routes or other-area work. The live B-04 game is the before baseline.'), '',finish['intent'],'',
...
def craft_schedule_notes(area):
    ...
    lines=['## Architecture and craftsmanship','',f"**Primary:** {schedule['primary']}", '',f"**Supporting:** {schedule['supporting']}", '',f"**Quiet fields and limits:** {schedule['quiet']}", '',
...
def craftsmanship_notes(design):
    craft=design['craftStandards'];lines=[f"# {design['issue']['revision'].split('-')[0]} | Bazaar architecture and craftsmanship",'',craft['benchmark'],'',
    ...
    alignment=craft.get('upperOpeningAlignment',{})
    if alignment:
        lines += ['', '## Upper window and door alignment', '', *[f"**{key}:** {md(value)}" for key,value in alignment.items() if isinstance(value,str)], '']
...
def architecture_atlas_svg(design,coverage_doc):
    ...
    parts=svg_header(2400,2485,design['issue']['revision'].split('-')[0]+' | Bazaar architecture and craftsmanship')
    ...
    issue={**design['issue'],'id':'BZ-04 / '+design['issue']['revision'].split('-')[0],'landmarks':design['landmarks']};coverage=coverage_index(coverage_doc);by_zone={a['zone']:a for a in design['areas']}
...
def elevation_svg(issue: dict[str, Any], area: dict[str, Any], coverage: dict[tuple[str, str], CoverageFace], materials: dict[str, Any], faces: Iterable[str] = FACE_ORDER) -> str:
    ...
    for face in faces:
        ...
        for index, parcel in enumerate(parcels):
            ...
            parts.append(f'<text class="parcel" x="{(x(run[0])+x(run[1]))/2:.2f}" y="{y(cap_top)-10:.2f}" text-anchor="middle">P{index+1}</text>')
...
def building_catalog(buildings: list[dict[str, Any]]) -> str:
    ...
    if revised:
        lines += ['', '## Complete-building intent', '',
...
def b_character_svg(area, materials, *, neutral=False):
    ...
    textblock('05  WHAT LEADS?', ('The retained merchant entrance and balcony form one address. Its matching upper timber-and-glass pair supplies a restrained second register; the quiet central field keeps the balcony prominent.' if revision in ('R4','R5','R6','R7') else 'The merchant entrance and balcony form one address. Patterned joinery is confined to its balustrade. Plain bedroom windows and the quiet upper center support that composition.'),882)
