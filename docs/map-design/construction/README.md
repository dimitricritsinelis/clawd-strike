...
Use this order as the coverage ledger, adjusting execution only for the handoff’s receiver/roof dependencies. Build named receiver shells before activating their bundles; install each shared owner once. Do not mark an area built while its prescribed receiver/interface remains unresolved. Record it as building, complete the queued dependency, and return to its consolidated interface check. Retain the last valid legacy output until its replacement is ready. At the end, reconcile all 25 units and shared owners, retire superseded temporary caps only with the supported final bundle, and verify whole-map seams and gameplay.

The user authorizes bounded subagents when they directly advance the current area or its required dependencies. Work one area at a time. Suggested models are GPT-6 Astra high for the lead and Astra medium for workers, not a hard worker limit. The lead owns integration, shared builders, runtime producers, registries and the completion ledger. Give each worker one bounded assignment with disjoint source/output ownership and explicit acceptance checks. Parallelize independent source work for that area; serialize heavy Blender exports, package application and live-game captures. Coordinate shared-helper changes before workers consume them.

Read each area and check its frozen scoped hash and builder capabilities once before its pass. Missing support is implementation work: extend the existing builder and meaningful fixtures to consume the approved schedule, then stage and validate exports before replacing valid assets. Do not skip features, disable geometry/integrity/gameplay guards, or reopen the design merely because the current builder lacks support. In particular, the current B builder accepts B-04; implementing R7 `B-05-finish-only` requires its scheduled finish/draped-rug support and assertions. Reuse unchanged B caps/shared assets. Later replace the caps atomically when the full Rug Gate bundle and receivers are installed, as the phase specifies.

Implement each complete area, perform one consolidated end review, correct observed defects and automatically continue. The implementation goal ends when every queued area/shared owner is installed and visually checked, required correctness and gameplay checks pass, and the final whole-map evidence is delivered for user review. Keep final artistic acceptance pending until the user grants it; do not turn that pending review into an automatic polling or rebuilding loop. Do not insert recurring design audits, approval ladders or routine human approval between areas. Actual-game visual, movement, collider, export identity and asset-load evidence remains required. Final user art acceptance is separate from construction and does not gate continuation. A concrete unresolved design contradiction or required external permission blocks only dependent work; continue independent authorized units and report the exact issue. On a user stop request, stop starting work promptly and preserve the last tested state.

**Construction-phase performance policy:** defer all performance benchmarking and optimization to a separate task after the full map is implemented. Preserve the numerical triangle, material, primitive, shadow and draw-call budgets as reference targets, including those called hard ceilings in generated sheets; they are informational during this phase and do not block export, integration or construction completion. Record counts when existing tooling supplies them. Do not seek budget increases, repeatedly measure CPU/FPS, or build new batching/atlas infrastructure to meet those targets. Reuse existing batching and `bz04-trial.mjs --capture-only`. Geometry validity, declared bounds, clear volumes, asset validity, load errors, gameplay identity and relevant regressions remain required. Record performance as deferred, never as passing.
...
3. At the end, perform one consolidated review: assemblies and receivers, exported clear volumes, matched game views, collider identity, movement, asset-load errors and relevant regressions. Use capture-only mode and establish final source/integrated/production hash equality and package idempotence in this pass.
...
pnpm build
node apps/client/scripts/bz04-trial.mjs --plan artifacts/b-next-trial/camera-plan.json --output artifacts/b-next-trial/before --capture-only
```

The before capture does not build or integrate new assets. The same runner captures after integration with `--capture-only`, a new `--output` directory and `--baseline artifacts/b-next-trial/before`; add `--movement` for the B courtyard’s exit and cover routes. `--views id,id` restricts an affected recapture to existing frozen poses. Use `--lighting flat` for a separate neutral material inspection; before/after comparisons must share the same lighting. It refuses to overwrite a capture directory. Capture-only mode preserves matched-camera, collider-authority, load-error and optional movement checks while skipping performance sampling and acceptance. Dated artifact scripts are historical records; reuse the tracked runner.

A B-04-compatible section build uses `build.py -- section-only --handoff artifacts/b-next-trial/handoff.json` and `integrate.py --handoff <saved-handoff.json>`. Integration requires an intact frozen handoff whose unit and scoped input hash match the current source, and rejects unsupported phases before copying or writing. The legacy integrator supports only `B-04-only`; implementing R7 includes its section-only finish integration path, not bypassing this guard. Its default export also emits the two B-local caps. Do not use that builder for a later finish until it implements every scheduled feature: the current B-05 draped-rug requirement is deliberately rejected rather than silently ignored. Run `python3 assets/source/unit-spawn-b-courtyard/integrate.py --self-test` and `python3 assets/source/unit-spawn-b-courtyard/verify.py --self-test` for prewrite rejection and budget-reporting fixtures. Small fixture tests never install assets. Validate geometry, bounds and asset integrity before replacing `.blend`/GLB files; record actual GLB counts against the unchanged budget references without blocking on overruns during construction.
...
The approved target is ready to implement under the canonical queue above. Execution still needs an executable, bounded construction recipe; filling a builder capability gap is authorized implementation work. The named builder must support its feature types, preserve valid geometry and declared bounds, and show its material finish in a representative in-game witness view. A coordinate-complete drawing or plausible tint table alone proves none of those conditions.

Keep the existing target and ceiling values for later performance work. Record assembly counts already supplied by the source inventory or export; approaching or exceeding a budget does not trigger simplification, rebudgeting or an approval step during construction. Do not erase scheduled openings or decorative voids to meet a count.
...
The later, separate performance task uses one full deterministic update/render per browser animation frame, with 60 warmup and 180 measured frames per view. Browser cadence, CPU work, draw/triangle counts and physical-device performance are separate measures. Relative CPU acceptance requires matching measurement modes and at least 120 samples; an old render-only or capture-only baseline is unverified, not interchangeable. Report raw failures and distribution overlap; repeat only a predefined affected measurement when needed, never until it passes. Do not run those benchmarks as part of the current construction queue.
