...
These are the canonical construction handoff for the Bazaar map, with one sheet per named area plus shared links and skyline schedules. An implementer (Codex, Claude, a person) builds what the sheet says; the layout and named character decisions are made here. SD-21/22 delegate bounded craft finish, not new architecture or free prop scatter. Use a cited standard detail only for an implementation detail the sheet delegates. If a required design choice or protected measurement is absent or conflicts with the spec, mark the area blocked in the progress index; do not invent a feature.
...
2. The sheets own design decisions: assemblies per bay, sills and heads, datums, awnings, signs, goods, overhead ends, wear, roofs, and each area's character schedule. Character schedules replace only the finish values they explicitly name (for example awning sag); all other task dimensions and protected values stand. A spec building brief or anchor note is context, not an additional construction task; the current role text and ordered tasks resolve historical prose such as Service South's obsolete vents/hatch.
...
4. [`../quality-bar.md`](../quality-bar.md) is the finish standard and `AGENTS.md` the art direction. The founding Bazaar image owns the warm aged surface and trade-patina target; district studies own geometry, craft and trade role only. Images never change construction measurements or scheduled work. The 2026-09-07 AI preview boards are interpretations of the previous sheets, not finish targets: their sterile paving, repeated cabinets, oversized arches and uniform pale blocks are not adopted.

The sheets are generated from live compiled geometry and placements by [gen.py](gen.py) with the design layer in [overlay.py](overlay.py). No schedule snapshot or archive is a construction authority. Regenerate after a design-layer edit or a spec change that moves a wall or a bay (`python3 docs/map-design/construction/gen.py`); edit the design in `overlay.py`, not in the generated files.
...
| SD-05 | Door | Stone jambs 0.14 wide × 0.12 proud, head 0.14; visible plank front at out=-0.05. Planks 0.04 deep occupy out=-0.09..-0.05; backing front at -0.095 or behind, never coplanar with plank fronts. Seams 0.002..0.003 wide reveal a sealed dark back, not black slots. Timber grain follows each plank; straps use narrow iron, not stone-like strips. Residential leaves use `ph_rough_pine_door`, two iron straps and a ring pull at 1.05; store leaves use `ph_weathered_brown_planks`, three straps and a 0.06 × 0.06 bumper at z 0.35. Threshold width = opening + 0.28, thickness 0.04, top z 0.01; it creates no collision step. Doors stay closed. Sheet-specific plank counts and hardware take precedence. |
| SD-06 | Window | Stone frame 0.10; sill top = the table sill datum, underside = datum minus 0.06, front = out 0.08. Reveal extends to out=-0.135, backed by dark plaster; closure per variant (kit shutters or a placed shutter/screen). Dark recess windows: closed dark timber leaf. |
...
| SD-12 | Wear | Use `Wall.wear(along, z0, width, height, kind, out=.025)` for wall marks and `wear_patch(F, corners, kind)` for the exact Section 6 floor calls. Both use untextured feathered vertexRGBA PBR roughness. Kinds: `dust`, `damp`, `polish`, `dye`, `spice`, `bleach`, `rut`. A named dirt band 0..1.5 m is a maximum application envelope: broken accumulation at feet, mostly 0..0.35, with isolated handling marks higher up. Never fill that rectangle or darken an entire wall uniformly; damp 0..0.8 on retaining walls; streak 0.25 wide × ≤ 0.45 under each spout; hand polish 0.9..1.4 at door jambs; cart scuffs 0..0.6 at store doors; dye only under workstations and racks; bleach only on exposed fields explicitly scheduled by the sheet (including Spice west's east-facing morning field). `rut` is 0.05 m wide wheel-scuff shading, never floor displacement. |
...
| SD-21 | Hand finish and repair | Preserve every envelope, axis, sill/head, structural bearing and support plane. On newly authored work, dressed stone arrises 0.006..0.012 m, rubble arrises 0.010..0.018, timber 0.003..0.006; clamp the bevel to less than one-quarter of the part's shortest dimension. Small irregular edge losses stay inside the original solid, never enlarge a gap at a seat. Plaster normal detail is 0.003..0.008 m apparent relief; trowel marks run in loose 0.15..0.40 m sweeps. Scheduled repairs replace the original skin inside their printed bounds, feather 0.03..0.06 m inward and retain its face plane. Break the repair edge within the bound, not into adjacent frames. No extra wall slab, displaced gameplay surface or crooked load-bearing frame. |
| SD-22 | Woven panel | Only at scheduled positions. Use the named existing cloth/carpet texture with visible warp/weft, hems and attachments; preserve its pattern and hue. Grain follows the weave, not world vertical by accident. Wall-pack cloth uses its recorded scale. The Project-Original rug uses the scale below; never stretch its pattern to fit a panel. Model the scheduled thickness/drape inside the envelope; secure each corner to the named receiving surface. Floor rugs, hanging galleries and rolled stock remain distinct assemblies. KEEP textiles are not deformed, recolored or moved. |
...
- Plaster: `ph_painted_plaster_warm`, `ph_beige_wall_002`, `ph_plastered_wall`, `ph_aged_plaster_ochre`, `ph_lime_plaster_sun`, `ph_whitewashed_brick_warm`, `ph_whitewashed_brick_cool`, `ph_red_plaster_weathered` (Spice north household), `ph_worn_plaster_sun` (worn sandy repairs), `ph_worn_plaster_ochre` (dark backs).
- Timber: `ph_rough_pine_door`, `ph_weathered_brown_planks`, `ph_worn_planks`, `ph_dark_wood`.
- Cloth: `ph_hessian_230` (cream awnings and canopies), `ph_fabric_leather_02` (tea house), `ph_dirty_carpet` is a coarse brown utility scan, not the scheduled patterned rug. Textile motifs come from the retained assets and the named Project-Original rug source below.
- Metal: `ph_rusty_metal_02`, `ph_painted_metal_shutter`.

For the new Spice panels, use `apps/client/public/assets/textures/environment/bazaar/textiles/project_original/levantine_rug_albedo_v1.jpg`, licensed Project-Original with recorded md5 in `textiles/sources.json`. Create an embedded PBR material named `bazaar_levantine_panel` (not a `ph_*` id), metallic 0, roughness 0.86, unmodified albedo. Map at 1.20 m per full texture repeat in both weave directions, crop at the hem rather than stretch, and keep the material images through `export_section`. The existing texture provides visible woven detail; no procedural colored bands or newly invented motif. This reads an existing texture only and does not alter any KEEP asset or its material.
...
The target is an active, flourishing Middle Eastern marketplace: sound buildings, cared-for stock, saturated cloth accents, heavy foot traffic and visible maintenance. Earth-toned, gritty plaster and aged lime sit beside cut and rubble sandstone. A dirt-colored wall is a material choice; a dirty wall needs a local cause. Occasional faded earth-red household plaster broadens the palette. Warm cream/tan unifies the district without making every building the same color or texture.

- **Building scale:** preserve the measured layout, inherited heights and existing off-axis features. Different tenancies have different plaster grain, colors, repair boundaries and shop treatments where their character schedules say so. Keep civic symmetry where it belongs. Do not force identical awnings, cabinets or surface histories onto a measured bay rhythm.
- **Hand scale:** use SD-21 stone arrises, timber grain, trowelled plaster and repaired edges; SD-22 textiles soften the street with weight, hems, sag and visible ties. Straight engineering datums do not require every visible edge to look machine-cut. Never tilt whole buildings, move openings or roughen support planes to simulate age.
- **Life and use:** retain stocked counters, rugs, roll chests, folded fabric, pottery, benches, planted sills and laundry at their scheduled locations. New character is explicitly listed, including Spice's fitted woven back panels. No crowds, animals, audio, new trades or props are implied by “life.” Quiet service faces communicate care through repairs and drainage, not retail clutter.
- **Traffic and dirt:** the busiest walking line is swept and polished, with readable paving joints; dust remains in mortar joints, sheltered corners and object contacts. Lower-wall wear breaks at swept thresholds. Moisture follows a real drain or wet workstation, spice residue stays at spice handling, and linen is used but intact. Keep most wall area free of added dirt; no sewage, garbage, rubble heaps, uniform brown wash, sepia lighting or full-height random grunge.
- **Deterministic craft discretion:** exact feature counts, materials, positions and maximum envelopes are fixed. Within them, the implementer chooses non-repeating stone edges, plank grain origins, trowel boundaries and wear masks under SD-21/22. Record the actual values or a fixed seed keyed by area/bay in `build.py`; never generate a fresh layout on rebuild. Do not use one repeating mask or alternating color formula as the design of every building. This discretion needs no extra approval.

SD-12 wall bands and Section 6 polygons bound the finish, not opaque rectangular deposits. Reuse `Wall.wear` / `wear_patch` for feathered contact; for broad wear, shape the exported `COLOR_0` alpha with irregular swept gaps inside the given polygon. Keep the helper's peak alpha/roughness, use patches of 0.15..0.60 m along a base, and leave intervals of 0.20..0.80 m clear. Broad traffic polish varies continuously in opacity rather than becoming a straight painted stripe. These are render marks only. Clip every mark to the actual receiving surface and exclude apertures, trims on a different plane and neighboring ownership.

Material ids are not proof of finish: use existing albedo, normal and roughness at their recorded world scale. `ph_aged_plaster_ochre` and `ph_plastered_wall`, for example, share a scan; tint alone does not count as different grain. Spice deliberately combines the painted-plaster, beige-wall and red-plaster scans. Avoid per-plank texture rotations that turn vertical grain sideways. Shared helpers are construction tools, not a license to ignore the sheet: inspect their output dimensions and compose the exact detail when a helper differs.
...
Build the requested area only. Spice Street is the first construction trial; its existing package predates this character revision and has open construction defects recorded in the progress index. A documentation revision does not rebuild it or authorize another area.
...
3. Perform the bounded construction inspection below and correct defects in the requested area. Then set the row in [`../development-plan/README.md`](../development-plan/README.md) to `built`, then stop after a single-area request. Continue only through an explicit user-provided area queue. `built` awaits a later user-authorized gameplay, performance and aesthetic validation task. Do not create a separate brief or start validation loops. Headless Blender builds are required. `pnpm build`, `map:check`, gameplay captures and playtests are deferred unless the user explicitly asks for a bounded check or preview; record such a check with the task result, not as another planning document.
...
The example demonstrates the real North East Link wall, not an alternative design. For each wall, pass its printed aperture list to `Wall.skin` before adding plinth/course/frames; skin, plinth and course then leave the opening clear. The example is a starting frame, not proof that every helper matches every SD detail. In particular, current `Wall.window` puts its sill lower and farther out than SD-06, and `Wall.door` is not the complete SD-05 plank/hardware assembly. Compose the exact detail from existing kit parts; when shared-helper work is part of an implementation, verify its callers before changing a shared default. No helper implementation was corrected by this documentation revision. Wear corners in Section 6 already include the 0.014 m render offset and the zone-floor conversion. Do not add those offsets a second time. Every attachment/mark must name its receiver, plane, bounds and offset in the build. Height alone is insufficient: a set-back parapet is not the street-wall plane. Wall wear sits 0.005 m in front of its receiving face: use `out=.025` on a 0.02 m skin and `out=.125` on a 0.12 m door jamb; use the scheduled projection + 0.005 for other frames.

## Bounded construction inspection

This is part of constructing the requested area, not an aesthetic scoring loop or a new approval ladder. Reuse one neutral-light scene and correct concrete construction defects. Do not progress to another area while known construction defects remain.

1. Inspect one close-up of each distinct assembled detail, including a closed door, a seated window, each different trade recess, cloth support and any new surface treatment. Confirm dimensions, intended contacts, correct grain/scale, non-coplanar backing and front faces, and the named material. Counts, a successful export or a material name cannot establish these facts.
2. Inspect one assembled context view from a street end with both owned faces, ground and relevant retained shells, roofs, parapets, signs, overheads and dressing. Add a reverse view only for interfaces hidden in the first. Bring the actual retained geometry into the preview; an approximate box or an omitted roof does not demonstrate contact. If that context cannot be reproduced, state the missing interface evidence and keep the row `building` (or `blocked` for a concrete dependency/conflict), not `built`.
3. Inspect the exported GLB in its section frame as well as the source assembly. Report checks by their real coverage: `map:check` establishes protected spec/placement constraints, not section-triangle clearance. A geometry check must test triangle/volume intersections against the protected walking and door-service volumes, with only the scheduled flush render marks and retained waivers exempted. Vertex-only endpoint tests can miss a face crossing a route. An absent geometry check is reported as unavailable, never inferred from `map:check`.
4. Use neutral daylight for the material comparison: fixed camera, Blender AgX, exposure 0, gamma 1, white light, no warm grade, fog or bloom. Record the light/world values and renderer in the preview script so successive assemblies are comparable. Show a wall/trim/timber/cloth close-up with a 1 m scale reference outside the exported scene. The founding reference sets the appearance; the tracked Spice proof only demonstrates the authoring path. A new approved finish reference does not exist yet.

Record the evidence and limitations in the task result, with a short note in the existing progress index where a defect prevents `built`. Do not create another scorecard, brief or status file. `built` means scheduled construction and its assembly/interface inspection are demonstrated; `complete` still requires later gameplay, performance and in-game aesthetic validation. Report export, application, construction evidence and release validation separately; avoid unsupported percentage-complete claims.
