...
Construction sheet. Read [README.md](README.md) first: it holds the coordinate conventions, the standard details (SD-xx), the clearance rules and the completion checklist that every task below assumes. Numbers here are design metres from `map_spec.json`; `pnpm map:shoot unit-covered-souk --tag r1-before` prints the same walls and must agree. Also called: Covered Souk, Covered Dyers Souk, the souk.
...
Three fabric trades under repaired arches on the east (packing, the approved booth, dye samples), the merchant block's trade arch and north-wing door on the west, one shared canopy overhead, the 5 m structural end wall at the north. The B18 pilot roof room sits on the east roof.
...
- **Corners:** `held`; solid end piers reserved: 1.56 m at a=0, 1.56 m at a=L. Ground head datum 3.55 m.
- **Horizontal datums (SD-01..SD-03 unless overridden):** plinth 0..0.28; impost 1.93; sign band 3.67..3.97; no continuous sill course (SD-06 sills at 4.09); coping 6.84..7.0.

| Bay | Module / assembly | Variant | a (m) | World (x, y) | W × D × H (m) | Sill / head (m) | Storey | Dressing bound here |
|---|---|---|---:|---|---|---|---:|---|
| `STORY_1_WINDOW_01` | `window_screened` → `ASMB_SCREEN_WINDOW` | SC-C fine crossed lattice (placed `ASSET_SCREEN_SC_C`) | 1.1 | (41, 34.38) | 1 × 0.24 × 1.4 | 4.15 / 5.55 | 1 |  |
| `GROUND_01` | `arch_arcade` | DY-S dye-sample counter (placed `ASSET_B18_DYE_COUNTER`) | 2.86 | (41, 36.14) | 2.6 × 0.42 × 3.55 | 0 / 3.55 | 0 | ASSET_B18_DYE_COUNTER (south-wing GROUND_01, unit scale on its threshold) |
| `STORY_1_WINDOW_02` | `window_screened` → `ASMB_SCREEN_WINDOW` | SC-C fine crossed lattice (placed) | 4.62 | (41, 37.9) | 1 × 0.24 × 1.4 | 4.15 / 5.55 | 1 |  |
...
1. CREATE the face GLB: skin `ph_aged_plaster_ochre`, plinth sandstone, impost band, coping; end piers 0.45 × 0.16 (`held`). Completion: the trade side of the central merchant block.
...
5. CREATE sign brackets for `DYE_W_SIGN_1` (a=2.86, board at z 3.67..3.97) at z 3.82, ±0.9. Completion: one sign.
...
- **Materials (profile `covered_arcade_wash`):** wall `ph_whitewashed_brick_cool`, trim `ph_stone_trim_sandstone`, roof `ph_worn_plaster_sun`, timber `ph_rough_pine_door`, metal `tm_balcony_painted_metal`, accent `ph_band_plastered`.
- **Corners:** `open`; solid end piers reserved: 0.79 m at a=0, 0.79 m at a=L. Ground head datum 2.7 m.
...
| `STORY_1_WINDOW_01` | `window_screened` → `ASMB_SCREEN_WINDOW` | SC-C fine crossed lattice (placed) | 1.36 | (41, 45.36) | 1 × 0.24 × 1.4 | 4.15 / 5.55 | 1 |  |
...
1. CREATE the face GLB: skin `ph_whitewashed_brick_cool` (a cooler wash for the north wing: the correlated difference), plinth, course 3.29..3.41, coping; end piers 0.45. Completion: reads as the same block's north wing across the passage.
...
- **Role:** public front. Three repaired stone arches: packing south, the approved textile booth at center, dry dye samples north; one broad closed roof-access room.
...
- **Corners:** `held`; solid end piers reserved: 0.6 m at a=0, 0.6 m at a=L. Ground head datum 3.55 m.
- **Horizontal datums (SD-01..SD-03 unless overridden):** plinth 0..0.28; impost 1.93; sign band 3.67..4.10 (3.80 over the booth); coping 4.34..4.5; slab 4.76, parapet cap 5.59; B18 roof-access room behind (placed).

| Bay | Module / assembly | Variant | a (m) | World (x, y) | W × D × H (m) | Sill / head (m) | Storey | Dressing bound here |
|---|---|---|---:|---|---|---|---:|---|
| `GROUND_01` | `arch_arcade` | packing bay (placed `ASSET_B18_PACKING_FINISH`) | 1.9 | (53, 35.18) | 2.6 × 0.42 × 3.55 | 0 / 3.55 | 0 | ASSET_B18_PACKING_FINISH (south GROUND_01, on the retained threshold) |
| `GROUND_02` | `arch_arcade` | approved textile booth (placed `ASSET_TEXTILE_BOOTH`, untouchable) | 6.72 | (53, 40) | 2.6 × 0.42 × 3.55 | 0 / 3.55 | 0 | ASSET_TEXTILE_BOOTH (center GROUND_02; approved file and placement unchanged) |
| `GROUND_03` | `arch_arcade` | DY-S dye-sample counter (placed `ASSET_B18_DYE_COUNTER`) | 11.54 | (53, 44.82) | 2.6 × 0.42 × 3.55 | 0 / 3.55 | 0 | ASSET_B18_DYE_COUNTER (north GROUND_03, front-served locked sample display) |
...
1. CREATE the face GLB: skin `ph_painted_plaster_warm`, plinth sandstone, impost band between the three arches, coping 4.34..4.5; end piers 0.60 × 0.16. Completion: low lime arcade of three repaired arches.
...
4. CREATE sign brackets for `DYE_E_SIGN_1` (a=6.72, board at z 3.80..4.10, brackets z 3.95) and `DYE_E_SIGN_2` (a=11.54, board at z 3.67..3.97, brackets z 3.82). `GROUND_01` has no sign. Completion: two signs.
5. CREATE a lantern bracket at a=7.22, z 3.75, 0.35 proud for the placed `LANTERN_DYERS_01` (52.5, 40.5, 4.1). Completion: lantern on a bracket.
...
1. CREATE the face GLB: skin `ph_sandstone_blocks_06`, plinth, course, coping 4.74..4.9 (closes `coping`); end piers 0.45. CREATE `BAY_01` niche at sill 1.30, a=2.1. One spout at a=0.5. Completion: a garden wall with one niche.
...
| `PLACE_B18_PACKING_FINISH_B18_PACKING_DISPLAY` | `ASSET_B18_PACKING_FINISH` | `B18_PACKING_DISPLAY` | (53.17, 35.18, 0.14) | 1.48 × 0.34 × 1.53 | 90 | KEEP at this transform |
...
| `PLACE_L34_COVERED_SOUK_BASKET_LMK_DYERS_DISTRICT` | `ASSET_CC0_BASKET` | `LMK_DYERS_DISTRICT` | (42.38, 43.82, 0) | 0.41 × 0.3 × 0.24 | 348 | KEEP at this transform |
| `PLACE_DYERS_LANTERN_LANTERN_DYERS_01` | `ASSET_CC0_LANTERN` | `LANTERN_DYERS_01` | (52.5, 40.5, 4.1) | 0.22 × 0.23 × 0.53 | 270 | KEEP at this transform |
...
| `PLACE_B4_SOUK_PROCESS_VESSEL_B4_SOUK_E_GOODS_GROUND_01` | `ASSET_DYERS_CERAMIC_VESSEL` | `B4_SOUK_E_GOODS_GROUND_01` | (51.98, 35.18, 0) | 0.55 × 0.42 × 0.31 | 277 | KEEP at this transform |
| `PLACE_DYERS_CERAMIC_VESSEL_LMK_DYERS_DISTRICT` | `ASSET_DYERS_CERAMIC_VESSEL` | `LMK_DYERS_DISTRICT` | (43.42, 43.88, 0) | 0.56 × 0.43 × 0.32 | 348 | KEEP at this transform |
| `PLACE_DYERS_VAT_EAST_LMK_DYERS_DISTRICT` | `ASSET_DYERS_SEALED_VAT` | `LMK_DYERS_DISTRICT` | (43.48, 44.5, 0) | 0.61 × 0.62 × 0.71 | 11 | KEEP at this transform |
| `PLACE_DYERS_VAT_WEST_LMK_DYERS_DISTRICT` | `ASSET_DYERS_SEALED_VAT` | `LMK_DYERS_DISTRICT` | (42.72, 44.42, 0) | 0.7 × 0.72 × 0.83 | 352 | KEEP at this transform |
| `PLACE_B4_SOUK_CART_B4_SOUK_W_CART_GROUND_01` | `ASSET_MARKET_CART` | `B4_SOUK_W_CART_GROUND_01` | (43.1, 36.14, 0) | 1.2 × 0.75 × 0.88 | 98 | KEEP at this transform |
| `PLACE_SUPPORT_CANOPY_DYERS_01_MOUNT_SUPPORT_CANOPY_DYERS_01` | `ASSET_ROOF_TIE_590` | `MOUNT_SUPPORT_CANOPY_DYERS_01` | (54.88, 45.36, 5.59) | 3.75 × 0.16 × 0.45 | 180 | KEEP at this transform |
| `PLACE_CENTRAL_SCREENS_COVERED_SOUK_CENTRAL_SCREEN_NORTH` | `ASSET_SCREEN_SC_C` | `CENTRAL_SCREEN_NORTH` | (40.98, 45.36, 4.15) | 1 × 0.24 × 1.4 | 270 | KEEP at this transform |
| `PLACE_CENTRAL_SCREENS_COVERED_SOUK_CENTRAL_SCREEN_SOUTH_1` | `ASSET_SCREEN_SC_C` | `CENTRAL_SCREEN_SOUTH_1` | (40.98, 34.38, 4.15) | 1 × 0.24 × 1.4 | 270 | KEEP at this transform |
| `PLACE_CENTRAL_SCREENS_COVERED_SOUK_CENTRAL_SCREEN_SOUTH_2` | `ASSET_SCREEN_SC_C` | `CENTRAL_SCREEN_SOUTH_2` | (40.98, 37.9, 4.15) | 1 × 0.24 × 1.4 | 270 | KEEP at this transform |
| `PLACE_DYERS_SIGNS_DYE_E_SIGN_1` | `ASSET_SIGNBOARD` | `DYE_E_SIGN_1` | (52.88, 40, 3.8) | 2.2 × 0.12 × 0.3 | 270 | KEEP (moved 2026-09-07 to z 3.80, board 0.30 high: 0.16 above the booth top 3.64, 0.24 under the coping 4.34) |
| `PLACE_DYERS_SIGNS_DYE_E_SIGN_2` | `ASSET_SIGNBOARD` | `DYE_E_SIGN_2` | (52.88, 44.82, 3.67) | 2.2 × 0.12 × 0.3 | 270 | KEEP (moved 2026-09-07 to z 3.67, board 0.30 high) |
| `PLACE_DYERS_SIGNS_DYE_W_SIGN_1` | `ASSET_SIGNBOARD` | `DYE_W_SIGN_1` | (41.12, 36.14, 3.67) | 2.2 × 0.12 × 0.3 | 90 | KEEP (moved 2026-09-07 to z 3.67, board 0.30 high: above the crown 3.55, under the screen frame 4.05) |
| `PLACE_TEXTILE_BOOTH_DYE_E_TEXTILE_BOOTH` | `ASSET_TEXTILE_BOOTH` | `DYE_E_TEXTILE_BOOTH` | (52.73, 40, 0) | 2.68 × 1.29 × 3.64 | 90 | KEEP exactly (user-approved at this location; never move, scale or re-export) |
...
|---|---|---|---|---|
| `CANOPY_DYERS_01` | (41, 45.36, 5.9) | (53, 45.36, 5.9) | 4.4 | Construction sheet 2026-09-07: both seats at 5.90, the west on a ledger above the north-wing screen (head 5.55), the east on a roof tie (ASSET_ROOF_TI |
...
East: slab 4.76, parapet 5.59, the placed roof room (cap 7.35) and the two vent boxes; west: 7.0 / 8.19 shared with the merchant house (M01: this GLB adds no roof). The north end wall (x 41..46, y 48) is an arcade end wall: quiet stone field, coping at 7.0, zero arches.
